...
import { Download, Calendar, Filter, TrendingUp, Users, Wallet, Package, Heart, Eye, ChevronLeft, ChevronRight } from 'lucide-react'
...
export default function ReportesClient({ cajas }: { cajas: any[] }) {
  ...
  const cargarDatos = async () => {
    setLoading(true)
    setCurrentPage(1) // resetear paginación al aplicar nuevos filtros
    try {
  ...
  // Paginación
  const totalPages = Math.max(1, Math.ceil(data.filas.length / PAGE_SIZE))
  const paginaActual = Math.min(currentPage, totalPages)
  const filasPagina = data.filas.slice((paginaActual - 1) * PAGE_SIZE, paginaActual * PAGE_SIZE)
      ...
      <div className="bg-white rounded-2xl border border-forest-green-dark/10 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[950px]">
            ...
            <tbody className="divide-y divide-forest-green-dark/5 text-sm text-forest-green-dark">
              {loading ? (
                <tr><td colSpan={12} className="p-8 text-center text-forest-green-dark/50">Cargando datos...</td></tr>
              ) : data.filas.length === 0 ? (
                <tr><td colSpan={12} className="p-8 text-center text-forest-green-dark/50">No hay ventas en este periodo.</td></tr>
              ) : (
                filasPagina.map((f: any) => (
                  <tr 
                    key={f.id} 
                  ...
                  >
                    ...
                    <td className="p-4 text-right text-forest-green-dark/80">
                      {formatCurrency(f.subtotal ?? (f.cantidad * f.precio_unitario))}
                    </td>
                        ...
                        <div className="flex flex-col items-end">
                          ...
                          {f.codigo_descuento ? (
                            <span className="text-[10px] font-mono bg-emerald-100 text-emerald-800 px-1.5 py-0.2 rounded font-bold">
                              {f.codigo_descuento} {f.porcentaje_descuento ? `(${f.porcentaje_descuento}%)` : ''}
                            </span>
                          ) : f.porcentaje_descuento ? (
                            <span className="text-[10px] text-emerald-700">
                              ({f.porcentaje_descuento}% desc.)
                            </span>
                          ) : null}
                        </div>
                      ) : (
                        <span className="text-forest-green-dark/30">$0.00</span>
                      )}
                    </td>
                    <td className="p-4 text-right font-bold text-quetzal-blue">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3 px-1 text-sm text-off-white/60">
          <p>
            ...
            <span className="font-semibold text-off-white">
              ...
              {Math.min(paginaActual * PAGE_SIZE, data.filas.length)}
            </span>{' '}
            de{' '}
            <span className="font-semibold text-off-white">{data.filas.length}</span>{' '}
            registros
          </p>
