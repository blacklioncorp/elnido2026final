...
export default async function ConfirmacionPage({
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-lg flex-col items-center justify-center gap-6 px-4 py-16 text-center">
      ...
      {/* Banner de Ubicación y Cómo llegar */}
      <div className="w-full max-w-sm rounded-xl border border-conservation-gold/30 bg-conservation-gold/10 p-4 text-center space-y-3">
        <div className="flex items-center justify-center gap-2 text-conservation-gold">
          <MapPin className="h-5 w-5 shrink-0" />
          <p className="text-sm font-semibold text-off-white">
            Muestra tu QR en: C. Progreso S/N, Ixtapaluca
          </p>
        </div>
        <Link
          href="/contacto"
          className="inline-flex items-center justify-center gap-1.5 w-full rounded-lg bg-conservation-gold px-4 py-2 text-xs font-bold text-forest-green-dark hover:bg-yellow-400 transition-colors shadow-sm"
        >
          📍 Cómo llegar
        </Link>
      </div>
