...
export async function createDonacionCheckout(input: DonacionInput): Promise<DonacionResult> {
  ...
  const { tarjetaId, donanteNombre, donanteEmail, donanteUsername, monto, mensaje, esRecurrente, turnstileToken } = parsed.data

  const captcha = await verifyTurnstile(turnstileToken)
  if (!captcha.success) {
    return { error: captcha.error || 'Verificación de seguridad fallida. Intenta de nuevo.' }
  }
  ...
  try {
    ...
    await notificarErrorAdmin(err, 'donación')
...
export async function createDonacionGenericaCheckout(
  ...
  const { nombre, email, monto, esRecurrente, turnstileToken } = parsed.data
  ...
  try {
    ...
    await notificarErrorAdmin(err, 'donación genérica')
