...
export async function POST(request: Request) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (session.metadata?.producto === 'donacion') {
      ...
      // 1. Idempotencia: verificar si ya existe una donación con este stripe_session_id
      const { data: donacionExistente } = await admin
        .from('donaciones')
        .select('id')
      ...
      if (donacionExistente) {
        // Evento ya procesado — ignorar sin error
    ...
    if (compraId) {
      // Idempotencia: verificar si la compra ya fue procesada
      const { data: compraActual } = await admin
        .from('compras')
        .select('estado')
        .eq('id', compraId)
        .maybeSingle()

      if (compraActual?.estado === 'completado' || compraActual?.estado === 'activado') {
        // Ya procesada — ignorar sin error
        return NextResponse.json({ received: true })
      }
      ...
      if (membresia && membresia.tipo_producto_id) {
        ...
        updates.descuento_eventos = ((meta?.descuento ?? meta?.descuento_eventos) as number) || 0
      }
      ...
      // 5. Si se utilizó un código de descuento de un solo uso, marcarlo como usado autoritativamente
      const codigoDescuentoId = session.metadata?.codigo_descuento_id || (compra?.metadata as any)?.codigo_descuento_id
      const codigoDescuentoStr = session.metadata?.codigo_descuento || (compra?.metadata as any)?.codigo_descuento

      let emailComprador: string | null = null
      if (compra && compra.cliente_id) {
        const { data: c } = await admin.from('clientes').select('email').eq('id', compra.cliente_id).maybeSingle()
        if (c?.email) emailComprador = c.email
      }
      if (!emailComprador) {
        emailComprador = session.customer_email || session.customer_details?.email || null
      }
      ...
      if (codigoDescuentoId) {
        ...
        await admin
          ...
          .ilike('codigo', codigoDescuentoStr)
      }
    }
  }
