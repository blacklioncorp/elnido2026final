...
import { Heart, X, Loader2, Shield, Lock } from 'lucide-react'
...
export default function FaunaDonarCTA({
  ...
  const modalJSX = (
    <AnimatePresence>
      {modalAbierto && (
        <div className="fixed inset-0 z-[99999] flex items-end md:items-center md:justify-center">
          {/* Backdrop con opacidad completa para aislar la vista */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          {/* Modal Card / Drawer inferior en móvil */}
          <motion.div
            ...
            className="relative z-10 w-full md:max-w-lg bg-[#0B2B26] border-t md:border border-white/20 rounded-t-3xl md:rounded-3xl max-h-[92vh] md:max-h-[85vh] flex flex-col shadow-2xl overflow-hidden"
          >
            {/* Handle visual para móvil */}
            <div className="md:hidden w-12 h-1.5 bg-white/25 rounded-full mx-auto my-3 shrink-0" />

            {/* Header Modal */}
            <div className="flex items-center justify-between px-6 pb-4 pt-1 md:pt-6 border-b border-white/10 shrink-0 bg-[#0B2B26]">
              <div>
            ...
            <form id="donacion-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto overscroll-contain p-6 space-y-4 bg-[#0B2B26]">
              {/* Selector de Montos */}
              <div>
            ...
            <div className="shrink-0 p-5 pb-8 md:pb-5 border-t border-white/10 bg-[#0B2B26]">
              <button
                type="submit"
                form="donacion-form"
                disabled={loading}
  ...
  return (
    <>
      ...
      {/* Renderizado vía React Portal en document.body para evitar stacking contexts del layout */}
