...
export default function AudioAmbiental() {
  ...
  const [audioUrl, setAudioUrl] = useState('/audio/audio_comprimido.mp3')
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (data && data.valor) {
          // If value is a remote Supabase URL, prefer local compressed asset
          if (!data.valor.includes('supabase.co')) {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-52 md:bottom-24 right-4 sm:right-6 z-30 flex flex-col items-end">
        <AnimatePresence>
          {isOpen && (
            <motion.div
