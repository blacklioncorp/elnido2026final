...
export default function MembresiaCard({
  ...
  const esPopular = membresia.es_popular ?? false
  ...
  const beneficios = [
    {
      ...
      texto:
        (meta.accesos ?? 1) === 1
          ? '1 acceso incluido'
          : `${meta.accesos ?? 1} accesos incluidos`,
    },
    {
      icon: Wallet,
    ...
    {
      icon: Percent,
      texto: `${meta.descuento ?? meta.descuento_eventos ?? 15}% de descuento en eventos y talleres`,
    },
  ]
  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col rounded-2xl border bg-white p-6 shadow-sm ${
        esPopular
          ? 'border-conservation-gold shadow-md shadow-conservation-gold/20 ring-1 ring-conservation-gold'
          : 'border-forest-green-dark/10'
      }`}
    >
      {esPopular && (
        <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-conservation-gold px-4 py-1 text-xs font-bold uppercase tracking-wide text-forest-green-dark shadow-sm">
          Más popular
        </span>
      )}
